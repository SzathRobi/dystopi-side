...
jQuery(document).ready(function ($) {
	...
	var preloader = function () {
		$(
			".blog .post-content>a>img, .blog:not('.horizontal-carousel').three-columns li>a>img, .post.single .post-image img, .services-list a>img, .team-box img, .projects-list:not('.isotope') a>img, .re-preload>img, .re-preload>.image-wrapper>img"
		).each(function () {
	...
	$('.cost-slider-input').on('paste change keyup', function () {
		...
		if ($("[data-input='" + self.attr('id') + "']").length)
			setTimeout(function () {
				$("[data-input='" + self.attr('id') + "']").slider(
					'value',
					self.val()
				);
			}, 500);
	...
	$('#interior-renovation-cost').costCalculator({
		formula:
			'ir-square-feet*ir-walls+ir-square-feet*ir-floors+ir-doors-value+ir-windows-value',
		currency: '$',
	...
	$('#paver-walkway-cost').costCalculator({
		formula:
			'pw-area-width*pw-area-length*pw-block-paving+pw-area-width*pw-area-length*pw-surface+pw-stone-walling-value',
		currency: '$',
		currencyPosition: 'before',
		thousandthSeparator: ',',
		decimalSeparator: '.',
	...
	$('.testimonials-list').each(function () {
		...
		for (var i = 0; i < elementClasses.length; i++) {
			...
			if (elementClasses[i].indexOf('pause_on_hover-') != -1)
				pause_on_hover = elementClasses[i].replace(
					'pause_on_hover-',
					''
				);
	...
	$(".our-clients-list:not('.type-list')").each(function (index) {
		...
		$('.re-preloader_' + index).before(
			"<span class='re-preloader'></span>"
		);
	...
	var horizontalCarousel = function () {
		$('.horizontal-carousel').each(function (index) {
			...
			$('.re-preloader-hr-carousel_' + index).before(
				"<span class='re-preloader'></span>"
			);
			$('.re-preloader-hr-carousel_' + index).imagesLoaded(function (
				instance
			) {
				$('.re-preloader-hr-carousel_' + index)
					.prev('.re-preloader')
					.remove();
				$('.re-preloader-hr-carousel_' + index).fadeTo(
					'slow',
					1,
					function () {
						$(this).css('opacity', '');
				...
				var navigation = 1;
				var control_for = '';
				var elementClasses = $('.re-preloader-hr-carousel_' + index)
					.attr('class')
					.split(' ');
				for (var i = 0; i < elementClasses.length; i++) {
					...
					if (elementClasses[i].indexOf('navigation-') != -1)
						navigation = elementClasses[i].replace(
							'navigation-',
							''
						);
					...
					if (elementClasses[i].indexOf('control-for-') != -1)
						control_for = elementClasses[i].replace(
							'control-for-',
							''
						);
				}
	...
	$('.tabs').tabs({
		...
		activate: function (event, ui) {
			ui.oldPanel
				.find(
					"[name='submit'], [name='name'], [name='email'], [name='message']"
				)
				.qtip('hide');
		}
	});
	...
	var teamSocialControls = function () {
		...
		$('.team-box').hover(
			function () {
				...
				currentControls = $(this).find(
					".social-icons:not('.social-static')"
				);
	...
	$(window)
		.on('hashchange', function (event) {
			...
			if (hashSplit[0].substr(0, 11) != 'prettyPhoto') {
				...
				$(
					".testimonials-list, .our-clients-list:not('.type-list')"
	...
	function windowResize() {
		...
		$('.testimonials-list').trigger('configuration', [
			'debug',
			false,
			true
		]);
				...
				if (menu_position < topOfWindow) {
					if (!$('#re-sticky-clone').length)
						$('.header-container').after(
							$('.header-container')
								.clone()
								.attr('id', 're-sticky-clone')
								.addClass('move')
						);
				} else {
					$('#re-sticky-clone').remove();
				}
			} else $('#re-sticky-clone').remove();
		}
	}
	...
	//hint
	$(
		".comment-form input[type='text'], .contact-form input[type='text'], .comment-form textarea, .contact-form textarea, .search input[type='text'], .search_form input[type='text'], .cost-calculator-container input[placeholder]"
	).hint();
			...
			success: function (json) {
				...
				if (typeof json.isOk != 'undefined' && json.isOk) {
					if (
						...
						self.find("input[type='text'], textarea")
							.trigger('focus')
							.trigger('blur');
					}
				} else {
					...
					if (
						typeof json.error_name != 'undefined' &&
						json.error_name != ''
					) {
						self.find("[name='name']")
							.qtip({
			...
			if ($(this).hasClass('re-smart-column')) {
				...
				if (
					...
					if (typeof self.data('percentage-value') != 'undefined')
						self.css(
							'width',
							parseInt(self.data('percentage-value'), 10) + '%'
						);
					...
					for (var i = 0; i < elementClasses.length; i++) {
						if (elementClasses[i].indexOf('animation-') != -1)
							animation = elementClasses[i].replace(
								'animation-',
								''
							);
						if (elementClasses[i].indexOf('duration-') != -1)
							duration = elementClasses[i].replace(
								'duration-',
								''
							);
